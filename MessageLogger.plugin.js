	...
	return class MessageLogger {
		...
		initialize () {
			if (window.BDFDB && typeof BDFDB === "object" && BDFDB.loaded) {
				...
				let loadedGuilds = BDFDB.DataUtils.load(this, "guilds");
				this.saveGuilds(!BDFDB.ArrayUtils.is(loadedGuilds) ? [] : loadedGuilds);
				...
				BDFDB.ModuleUtils.patch(this, BDFDB.LibraryModules.DispatchApiUtils, "dirtyDispatch", {after: e => {
					if (BDFDB.ObjectUtils.is(e.methodArguments[0]) && e.methodArguments[0].type == BDFDB.DiscordConstants.ActionTypes.MESSAGE_CREATE && e.methodArguments[0].message) {
						...
						message.guild_id = message.guild_id || BDFDB.DiscordConstants.ME;
						if ((message.nonce || message.attachments.length > 0) && !logged.includes(message.id) && guilds.includes(message.guild_id)) {
		...
		saveGuilds (savedGuilds) {
			guilds = savedGuilds;
			BDFDB.DataUtils.save(savedGuilds, this, "guilds");
		}
		...
		showLogs () {
			...
			let logs = [];
			let filepath = BDFDB.LibraryRequires.path.join(this.logsfolder, channel.guild_id || BDFDB.DiscordConstants.ME, channel.id + ".txt");
			if (BDFDB.LibraryRequires.fs.existsSync(filepath)) logs = BDFDB.LibraryRequires.fs.readFileSync(filepath).toString().split("\n");
			
			if (!logs.length) BDFDB.NotificationUtils.toast("No logs saved yet", {type: "error"});
													...
													onClick: _ => {
														...
														for (let file of files[1].split(" ")) if (file) links.push(BDFDB.ReactUtils.createElement(BDFDB.LibraryComponents.Anchor, {
															...
															onClick: e => {
																...
																let filepath = BDFDB.LibraryRequires.path.join(this.logsfolder, channel.guild_id || BDFDB.DiscordConstants.ME, channel.id + "_images", filename);
